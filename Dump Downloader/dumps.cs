...
using System.Linq;
using static Dump_Downloader.Http;
...
namespace Dump_Downloader
{
    public class Dumps
    {
        ...
        public static void DownloadDumps(List<string> dumpNames, List<string> dumpUrls, string userAgent, string nationOrRegion)
        {
            // Shell Progress Bar
            int totalTicks = dumpUrls.Count();
            ...
            for (int i = 0; i < dumpUrls.Count; i++)
            {
                // Safety check before continuing
                if (dumpNames.Count != dumpUrls.Count)
                {
                    throw new Exception($"ERROR: The list lengths of dumpNames and dumpUrls do not match. Program will refuse to continue.Names: {dumpNames.Count}, Urls: {dumpUrls.Count}");
                }
                ...
                ApiRequest($"https://nationstates.s3.amazonaws.com/{nationOrRegion}_dump/{dumpUrls[i]}", userAgent, $"{path}\\{dumpNames[i]}");
